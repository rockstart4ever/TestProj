//  Constants.h

#import <Foundation/Foundation.h>

#define kMenuTableCellHeight          40.0f
#define kCellX                        5.0f
#define kCellY                        5.0f
#define kResourceCellImageWidth       60.0f
#define kResourceCellImageHeight      60.0f
#define kResourceCellLinkLabelHeight  15.0f
#define kResourceCellHeight           70.0f
#define kImageViewTag                 10
#define kTextLabelTag                 11
#define kDetailLabelTag               12
#define kHeaderHeight                 100.0f
#define kNumberOfVideosInRow          2
#define kMarginBetweenVideoItems      10

#define kFontName                     @"Helvetica-Light"
#define kRegularFontName              @"Helvetica"
#define kBoldFontName                 @"Helvetica-Bold"

#define kBaseApi   @"http://demo.redsymbolhost.com/carvan/restApi/api.php"